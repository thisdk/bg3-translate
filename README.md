...
- **比什么**：比的是原文与译文的**结构签名**——占位符的多重集，加上白名单富文本标签
  （`<LSTag>` / `</LSTag>` / `<br/>`）的标签名**多重集**、开始标签的**属性名**，
  以及 **key 型属性的值**。标签之间的正文被翻译了不算问题：只比结构，不比内容。
  占位符有**两种写法**，各自独立比对（`{1}` 不能顶 `[1]`）：

  - 花括号：`{1}` / `{10}` / `{name}` / `{user_name}`
  - 方括号：`[1]` / `[10]`（游戏替换数值）、`[IE_PanelSelect]` / `[DRUID]`（内部 ID）。
    这类是真实存在的一大类 —— 官方术语表里 163 条含 `[数字]` 的条目，
    **官方简中 163/163 全部原样保留**，其中 `, [1] from [2]` → `，从[2]处取走了[1]`
    还换了位，所以它和 `{N}` 一样是「顺序随意、多重集必须相等」。

  缺一处 / 多一处 / 重复都算问题。
- **属性值是查表 key，不是显示文本，必须逐字照抄**。真实语料里
  `<LSTag Tooltip="VENOMOUS_BARBS_CONDITION">Deadly Toxin</LSTag>` 的 `Tooltip`
  是游戏用来查表的内部 ID，正文 `Deadly Toxin` 才是显示文本 —— 也就是说
  翻属性值等于把 key 改掉，游戏查不到表，tooltip 直接失效。
  所以 `Type="Spell"` / `Tooltip="HitPoints"` 这类**标识符形态**的 key 值
  被翻译会报错。非 key 型属性（`Tag`、`color` 等）的值、以及 key 属性里
  含空格的自然语言值仍然自由 —— 后者是刻意的盲点：真实语料的 279 个
  `Tooltip` 取值一个空格都没有，收进来只会误伤。
- **标签顺序不算问题**：中英语序不同，标签跟着它包住的那段正文换位是常态。
  例如 `Inflicts <A>Deadly Toxin</A> ... fails a <B>Saving Throw</B>` 译成中文后
  条件从句提前、结果在后，两个标签必然对调——这是**正确译文**，不会被判失败。
  仍然要查的是**开闭不配对**（落单的闭合标签，或不同标签名之间交叉嵌套，
  写回层会把整条降级成纯文本，游戏里看到一堆字面标签）。
- **占位符不许粘连**：原文里分开的两个占位符被写成 `[1][2]` 会报错 ——
  游戏分别替换两个值之后中间没有任何分隔，会渲染成**一个数** `12`。
  这是内容缺陷而非排版问题。反过来，给它们之间**加**分隔永远放行
  （`[1] [2]`、`[1]、[2]`、`[1]，[2]` 都行）：多一个分隔无害，粘在一起才有害。
- **占位符周围的空格不参与比对**：`deal [2] damage` 译成「造成[2]点伤害」是正确中文，
  英文的词间空格不是结构。官方简中里 `[N]` 两侧带空格的比例只剩 6.9% / 3.4%
  （英文源是 61.3% / 38.7%），保留下来的都是载重的分隔符。
- **两类写法差异会被自动修回来，而不是判失败**（都发生在校验**之前**，
  修好的形态既进 `Done` 事件也进 PAK）：

  - **全角括号 → 半角**：模型按中文排版习惯写成 `【2】` 时修成 `[2]`；
    只修内层是合法占位符形态的，译文里作为标点用的 `【注意事项】` 不会被误伤。
  - **括号类型被改写**：模型把少见的 `[1]`「归一化」成它更熟悉的 `{1}` 时，
    按**原文**修回 `[1]`——这两套括号是游戏的两套替换机制，换了类型值就填不进去。
    只在**原文只使用了一种括号类型**时才修；原文两种都用时不猜（分不清谁是谁改的），
    交给校验报错。模型凭空多写的 `{2}` 也不会被洗白成原文有的那个。
...
- **防误报**：`HP < 5`、`a < b`、`<5>` 这类不良好的尖括号按普通文本处理；白名单之外的
  标签名（`<name>`、`<color>`）不算标签；混合大小写的方括号散文（`[Note]`、`[Draft]`）
  不算占位符；`&lt;` 会先还原成字面 `<` 再比较，
  模型把 `<` 转义回去也不会误判（这条防线拦的是结构丢失，不是转义风格——
  两者都拦会误伤大量正常译文）。
- **模型输出转义形态时由写回层兜底**：校验会放行 `&lt;LSTag&gt;` 这类转义写法，
  所以写回前会先做一次**实体还原再统一转义**：`&lt;LSTag&gt;` → 落盘 `&lt;LSTag&gt;`
  （游戏读回真标签）、`HP &lt; 5` → 落盘 `HP &lt; 5`（`<` 正常显示）、`&amp;` → `&`。
  两种标记编码风格分流处理：转义风格整条再转义，真元素风格跳过标签区间
  （否则标签属性里的实体会被还原成裸 `&`，产出非法 XML —— 这条正是独立验证
  抓到的回归，见 `docs/CORPUS-AUDIT.md` 的 D9）。
  两条已知取舍：源文件里真想显示 `&lt;` 的文本（`&amp;lt;`）**每保存一次降一层**；
  「真元素风格 + 整条标签被转义 + 属性里带 `&quot;`」三条件同时成立时标签会降级成
  字面文本（产物仍合法、不丢内容）。兜底只在**落盘**发生，界面上的 `Done` 事件
  仍是模型输出的转义形态。
  **属性名**是结构，必须逐字保留（写回时模型输出的标签会被原样写进 PAK，
  原文标签不会被恢复）。
...
crates/bg3-translate-core/     # 纯逻辑核心，零 GUI 依赖
  ...
  tests/real_mod_sample.rs     #   真实 Nexus MOD 样本的闭环 + 结构校验误报防线
  tests/corpus_regression.rs   #   1971 条真实 contentList 语料：9088 个防误报 / 防漏报用例
  tests/corpus_writeback.rs    #   同一份语料的写回字节往返与标记编码风格回归
samples/english.xml            # 真实 BG3 contentList（1971 条），上面两个语料测试的输入（只读）
src-tauri/                     # Tauri 薄壳：命令层 + 事件桥 + 插件注册
src/                           # React 前端
docs/ARCHITECTURE.md           # 架构说明与冻结的 IPC 契约
docs/CORPUS-AUDIT.md           # 真实语料独立验证：确认缺陷 / 刻意取舍 / 无法验证
...
### 真实语料回归（`samples/english.xml`）

上面那个真实 MOD 样本有个致命短板：41 条全是纯 UI 文案，`{`、`[`、`<` **一个都没有**，
所以结构保真这一层从来没有被真实数据碰过。`samples/english.xml`（1971 条真实
BG3 `contentList`，由用户从真实 MOD 提取）补上了这个缺口：

| 事实 | 数值 |
| --- | --- |
| 条目 / 平均长度 / 最长 | 1971 / 80.16 字符 / 581 |
| 标签种类 | 只有 `LSTag`（1103 对）与 `<br>`（410 个，**全部不带斜杠**） |
| 属性种类 | 只有 `Tooltip`（1103）与 `Type`（625，取值闭集 Spell/Status/Passive） |
| `[数字]` 占位符 | 303 条 / 417 处；`{...}` **0 处** |
| 含 ≥2 个 `LSTag`（可换位） | 288 条 |
| 标记编码形态 | `&lt;LSTag` × 1103、`&lt;br&gt;` × 410；真标签 **0 处** |
| XML 实体 / 裸尖括号 / 嵌套深度 | 0 / 0 / 最大 1 |

- `corpus_regression.rs`：**9088 个用例**。A 组零误报 6504（自反、标记逐字照抄换正文、
  尖括号重新转义、288 条标签整体换位、占位符空格增删）；B 组零漏报 2584（丢标签对、
  属性名改坏、占位符丢/改号/凭空加、`[N]`→`{N}`、相邻粘连、丢 `<br>`、开闭不配对）。
  朴素标记扫描器**自己实现**，不复用被测代码，两边独立才谈得上互相验证。
- `corpus_writeback.rs`：写回字节往返（**1945/1971 = 98.68%**，剩余 26 条全是预先存在的
  尾空格归一化）、标记编码风格**不被翻转**、生产链路（`write_entries`）确实走跟随风格那条。

语料是**只读输入**，缺失时测试**响亮失败**而不是静默跳过。它和
`Appearance Edit Enhanced-*.zip` 一样随仓库提交（非 Git LFS）。

`docs/CORPUS-AUDIT.md` 是基于这份语料的独立验证报告：确认缺陷、刻意取舍、
无法验证项各归各的，每条都带复现命令。
